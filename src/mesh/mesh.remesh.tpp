...
struct Mesh<VertData, TriData>::RemeshScratchpad {
    ...
    std::set< std::pair<double, TopoEdge*> >     queue;
...
void Mesh<VertData, TriData>::remesh()
{
    ...
    scratchpad.cache.edges.for_each([this, &scratchpad](TopoEdge* edge) {
        if(edge->tris.size() != 2) {
    ...
    while(scratchpad.queue.size() > 0) {
        ...
        TopoEdge*        top_edge        = it_top->second;
...
inline void Mesh<VertData, TriData>::edgeNeighborhood(
    ...
    TopoVert*                    v0          = edge->verts[0];
    TopoVert*                    v1          = edge->verts[1];
    VertData                &data0      = verts[v0->ref];
    VertData                &data1      = verts[v1->ref];
    
    once(data0, data1);
    
    for(TopoTri* tri : edge->tris) {
        TriData             &tdata      = tris[tri->ref].data;
        for(uint i=0; i<3; i++) {
            if(tri->edges[i] == edge) {
                TopoVert*        vopp        = tri->verts[i];
...
template<class VertData, class TriData>
double Mesh<VertData, TriData>::computeEdgeScore(TopoEdge* edge) {
    double edge_length;
...
void Mesh<VertData, TriData>::populateTriFromTopoTri(TopoTri* tri) {
...
template<class VertData, class TriData>
TopoEdge* Mesh<VertData, TriData>::allocateRemeshEdge(
    RemeshScratchpad &scratchpad
) {
    TopoEdge*        e           = scratchpad.cache.newEdge();
...
struct EdgeWedge {
    ...
    TopoEdge* one()  const { return (e0)? e0 : e1; }
};
...
struct TriWedge 
{
    ...
    TopoTri* one()  const { return (t0)? t0 : t1; }
};
...
struct VptrRemap 
{
    ...
    TopoVert* v1;
    TopoVert* v_merged;
    VptrRemap(TopoVert* v0_, TopoVert* v1_, TopoVert* v_merged_) :
        v0(v0_), v1(v1_), v_merged(v_merged_)
    {}
    TopoVert* operator[](TopoVert* key) {
...
void Mesh<VertData, TriData>::edgeCollapse(
     ...
     */
    ShortVec<TopoTri*, 2>       tris_collapse   = e_collapse->tris;
    
    TopoVert*                    v0              = e_collapse->verts[0];
    TopoVert*                    v1              = e_collapse->verts[1];
    
    ShortVec<EdgeWedge, 2>  edge_wedges;
    ShortVec<TopoEdge*, 10>      edges_moving;
    
    ShortVec<TriWedge, 2>   tri_wedges;
    ShortVec<TopoTri*, 14>      tris_moving;
    
    // BUILD all the edge wedges
    std::map<TopoVert*, EdgeWedge>   edge_w_map;
    for(TopoEdge* e : v0->edges) {
        if(e == e_collapse) continue;
        TopoVert*                ev0                 = e->verts[0];
        TopoVert*                ev1                 = e->verts[1];
        TopoVert*                key                 = (ev0 != v0)? ev0 : ev1;
                            edge_w_map[key].e0  = e;
    }
    for(TopoEdge* e : v1->edges) {
        if(e == e_collapse) continue;
        TopoVert*                ev0                 = e->verts[0];
        TopoVert*                ev1                 = e->verts[1];
        TopoVert*                key                 = (ev0 != v1)? ev0 : ev1;
                            edge_w_map[key].e1  = e;
    }
    ...
    std::map<TopoEdge*, TriWedge>    tri_w_map;
    for(TopoTri* t : v0->tris) {
        if(t->edges[0] == e_collapse ||
           t->edges[1] == e_collapse ||
           t->edges[2] == e_collapse)   continue;
        
        TopoEdge*                key                 = nullptr;
        for(uint k=0; k<3; k++)
            if(t->verts[k] == v0)
                            key                 = t->edges[k];
        ENSURE(key != NULL);
    ...
    TopoVert*                    v_merged            = scratchpad.cache.newVert();
    ShortVec<TopoEdge*, 2>       edges_merged(edge_wedges.size());
    ShortVec<TopoEdge*, 10>      edges_moved(edges_moving.size());
    ShortVec<TopoTri*, 2>       tris_merged(tri_wedges.size());
    ShortVec<TopoTri*, 14>      tris_moved(tris_moving.size());
    ...
    if(!collapsing_tetrahedra_disappear) {
        for(uint i=0; i<tri_wedges.size(); i++) {
            TopoTri*            t0                  = tri_wedges[i].t0;
            //TopoTri*            t1                  = tri_wedges[i].t1;
            TopoTri*            t_new               = tris_merged[i];
    ...
    for(uint i=0; i<tris_moving.size(); i++) {
        TopoTri*                t_old               = tris_moving[i];
        TopoTri*                t_new               = tris_moved[i];
    ...
    // Next, the edges
    for(uint i=0; i<edge_wedges.size(); i++) {
        TopoEdge*                e0                  = edge_wedges[i].e0;
        TopoEdge*                e1                  = edge_wedges[i].e1;
        TopoEdge*                e_new               = edges_merged[i];
        
        // plug in all the valid triangles...
        for(TopoTri* t : e0->tris) {
                            t                   = tptr_remap[t];
            if(t)           e_new->tris.push_back(t);
        }
        for(TopoTri* t : e1->tris) {
                            t                   = tptr_remap[t];
            if(t)           e_new->tris.push_back(t);
        }
    ...
    for(uint i=0; i<edges_moving.size(); i++) {
        TopoEdge*                e_old               = edges_moving[i];
        TopoEdge*                e_new               = edges_moved[i];
        
        // note: should never have any dead/null triangles
        for(TopoTri* t : e_old->tris) {
                            t                   = tptr_remap[t];
            ENSURE(t);
    ...
    ShortVec<TopoEdge*, 16>      borderEdges;
    for(const TriWedge &e_wedge : tri_wedges) {
        TopoTri*                t0                  = e_wedge.t0; 
        for(uint k=0; k<3; k++) {
            if(t0->verts[k] == v0) {
    ...
    ShortVec<TopoEdge*, 12>      new_edges;
    ShortVec<TopoTri*, 16>      new_tris;
    {
        for(TopoEdge* e : edges_merged)
            if(e)           new_edges.push_back(e);
        for(TopoEdge* e : edges_moved) // cannot be deleted
                            new_edges.push_back(e);
        for(TopoTri* t : tris_merged)
            if(t)           new_tris.push_back(t);
        for(TopoTri* t : tris_moved) // cannot be deleted
                            new_tris.push_back(t);
    }
    // hook up edges to existing geometry
    for(TopoEdge* edge : new_edges) {
        TopoVert*                ev0                 = edge->verts[0];
        TopoVert*                ev1                 = edge->verts[1];
        TopoVert*                v_old               = (ev0 != v_merged)? ev0 : ev1;
                            v_old->edges.push_back(edge);
    }
    // hook up triangles to existing geometry
    for(TopoTri* tri : new_tris) {
        for(uint k=0; k<3; k++) {
            if(tri->verts[k] != v_merged)       continue;
            TopoEdge*            e                   = tri->edges[k];
            TopoVert*            tv0                 = e->verts[0];
            TopoVert*            tv1                 = e->verts[1];
     ...
     */
    ShortVec<TopoTri*, 16>      dead_tris;
    ShortVec<TopoEdge*, 16>      dead_edges;
    ShortVec<TopoVert*, 2>       dead_verts;
    ...
    for(TopoTri* t : tris_moving)
                            dead_tris.push_back(t);
    
    // process the list of triangles
    for(TopoTri* tri : dead_tris) {
    ...
    for(TopoEdge* edge : dead_edges) {
        // Let's unhook this edge from its vertices
        for(uint k=0; k<2; k++) {
            TopoVert*            v                   = edge->verts[k];
                            remove(v->edges, edge);
    ...
    for(TopoEdge* e : edges_merged) { if(e) { // might be deleted
                            scoreAndEnqueue(scratchpad.queue, e);
    }}
    for(TopoEdge* e : edges_moved) { // def. not deleted
                            scoreAndEnqueue(scratchpad.queue, e);
    }
    for(TopoEdge* e : borderEdges) {
...
void Mesh<VertData,TriData>::edgeSplit(
    ...
    */
    ...
    ShortVec<TopoTri*, 2>       ts_orig             = e_split->tris;
    
    TopoVert*                    v0                  = e_split->verts[0];
    TopoVert*                    v1                  = e_split->verts[1];
    
    ShortVec<TopoVert*, 2>       vs_opp(ts_orig.size());
    for(uint i=0; i<ts_orig.size(); i++) {
        TopoTri*                t_orig              = ts_orig[i];
        for(uint k=0; k<3; k++) {
     ...
     */
    TopoVert*                    v_new       = scratchpad.cache.newVert();
    TopoEdge*                    e0_new      = allocateRemeshEdge(scratchpad);
    TopoEdge*                    e1_new      = allocateRemeshEdge(scratchpad);
    ShortVec<TopoTri*, 2>       t0s_new(ts_orig.size());
    ShortVec<TopoTri*, 2>       t1s_new(ts_orig.size());
    ShortVec<TopoEdge*, 2>       es_mid(ts_orig.size());
    ...
    for(uint i=0; i<ts_orig.size(); i++) {
        TopoTri*                t_orig              = ts_orig[i];
        TopoTri*                t0                  = t0s_new[i];
        TopoTri*                t1                  = t1s_new[i];
        TopoEdge*                e_mid               = es_mid[i];
        
        // replace every edge and vertex appropriately for the two variants
        for(uint k=0; k<3; k++) {
            TopoVert*            v_orig              = t_orig->verts[k];
            TopoEdge*            e_orig              = t_orig->edges[k];
            if(v_orig == v0) {
                            t0->verts[k]        = v_orig;
                            t0->edges[k]        = e_mid;
                            t1->verts[k]        = v_new;
    ...
    ShortVec<TopoEdge*, 4>       borderEdges;
    for(TopoTri* t : ts_orig) { // TODO: evacuate all of this to the end...
        for(uint k=0; k<3; k++) {
    ...
    for(uint i=0; i<ts_orig.size(); i++) {
        ...
        for(uint k=0; k<3; k++) {
            if(t_orig->verts[k] == v0) {
                TopoEdge*        e1                  = t_orig->edges[k];
                            e1->tris.push_back(t1s_new[i]);
            } else if(t_orig->verts[k] == v1) {
                TopoEdge*        e0                  = t_orig->edges[k];
